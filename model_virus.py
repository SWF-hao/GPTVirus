...
class GPTConfig:
    ...
    species_vocab_size: int = 14191 # number of species in the vocabulary, if applicable
    use_species_embeddings: bool = True # whether to use species embeddings or not
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.use_species_embeddings:
            self.species_embedding = nn.Embedding(config.species_vocab_size, config.n_embd)
    ...
    def forward(self, idx, targets=None, species_ids=None):
        device = idx.device
        b, t = idx.size()
        ...
        if self.config.use_species_embeddings and species_ids is not None:# add species embeddings if applicable
            species_emb = self.species_embedding(species_ids)  # (B, D)
            # species_emb = species_emb.unsqueeze(1).expand(-1, t, -1)  # (B, T, D)
            tok_emb = tok_emb + species_emb
